...
export function LoginForm({
  fixtureMode,
  playtestBypass = false,
}: {
  fixtureMode: boolean;
  /** TEMPORARY — remove with PLAYTEST_AUTH_BYPASS before public launch. */
  playtestBypass?: boolean;
}) {
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
        {playtestBypass
          ? "Playtest mode — enter without email."
      ...
      {playtestBypass ? (
        <div className="mt-6">
          ...
          >
            Enter playtest (no email)
          </Button>
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            Temporary playtest entrance — remove before public launch.
          </p>
        </div>
      ) : null}
      ...
      {!playtestBypass ? (
        ...
        <details className="mt-8 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <div className="my-4 flex items-center gap-3 text-[11px] tracking-wide uppercase">
            <span className="h-px flex-1 bg-zinc-200 dark:bg-zinc-800" />
            or
            <span className="h-px flex-1 bg-zinc-200 dark:bg-zinc-800" />
          </div>
